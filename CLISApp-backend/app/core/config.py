...
class Settings(BaseSettings):
    ...
    # Data Sources
    copernicus_cds_api_key: Optional[str] = Field(
        default=None, env="COPERNICUS_CDS_API_KEY"
    )
    copernicus_cds_url: str = Field(
        default="https://cds.climate.copernicus.eu/api/v2", env="COPERNICUS_CDS_URL"
    )
    ...
    processing_path: Path = Field(
        default=Path("./data/processing"), env="PROCESSING_PATH"
    )
    firebase_credentials_path: Path = Field(
        default=Path("./data/firebase-service-account.json"),
        env="FIREBASE_CREDENTIALS_PATH",
    )
    health_thresholds_path: Path = Field(
        default=Path("./data/health_thresholds.json"),
        env="HEALTH_THRESHOLDS_PATH",
    )
    ...
    def queensland_bounds(self) -> dict:
        ...
        return {"north": -10.0, "south": -29.0, "east": 154.0, "west": 138.0}
