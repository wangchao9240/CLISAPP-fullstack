"""
Open-Meteo Temperature Processing Module

Fetches temperature data from Open-Meteo API and creates GeoTIFF
...
Pipeline:
1. Fetch temperature data from Open-Meteo API
2. Create 2D numpy array from grid points
3. Convert to GeoTIFF (Web Mercator projection)
...
class OpenMeteoTemperatureProcessor:
    """
    Process Open-Meteo temperature data to GeoTIFF.

    Much faster than MODIS - typically completes in under 60 seconds.
    """
    ...
    # Queensland bounds (same as MODIS processor)
    ...
    def __init__(
        ...
        """
        Initialize Open-Meteo temperature processor.
        ...
        self.output_dir = self.data_root / "processing" / "temp"
        ...
        logger.info(
            f"Initialized Open-Meteo temperature processor "
    ...
    async def _fetch_batch(
        ...
        """Fetch temperature data for a batch of coordinates with retry logic."""
        import httpx
    ...
    async def fetch_temperature_data(self) -> Optional[np.ndarray]:
        """
        Fetch temperature data from Open-Meteo API.

        Returns:
            2D numpy array of temperature values, or None if failed
        """
        import httpx

        logger.info(f"Fetching temperature data for {len(self.grid_points)} grid points...")
                ...
                if result:
                    ...
                    if isinstance(result, list):
                        for i, point_data in enumerate(result):
                            point = batch[i]
                            temp = point_data.get("current", {}).get("temperature_2m")
                            if temp is not None:
        ...
        if valid_pixels == 0:
            logger.error("No valid temperature data received")
            return None
    ...
    def save_to_geotiff(self, data: np.ndarray, output_path: Path) -> bool:
        """
        Save temperature data to GeoTIFF.
    ...
    def process_full_pipeline(self) -> Optional[Path]:
        """
        Run the complete Open-Meteo temperature processing pipeline.

        Returns:
            Path to output GeoTIFF, or None if failed
        """
        logger.info("Starting Open-Meteo temperature processing pipeline")
        start_time = datetime.now()

        try:
            # Step 1: Fetch data
            logger.info("Step 1/2: Fetching temperature data from Open-Meteo...")
            data = asyncio.run(self.fetch_temperature_data())
            ...
            output_path = self.output_dir / f"temperature_openmeteo_{timestamp}.tif"
            ...
            latest_link = self.output_dir / "temperature_latest.tif"
...
def process_temperature_openmeteo(
    data_root: Optional[Path] = None,
    resolution: float = 0.5
) -> Optional[Path]:
    """
    Convenience function to process Open-Meteo temperature data.
    ...
    Returns:
        Path to output GeoTIFF, or None if failed
    """
    processor = OpenMeteoTemperatureProcessor(
        data_root=data_root,
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("Open-Meteo Temperature Processing Pipeline")
    print("="*60)

    result = process_temperature_openmeteo()
